...
import { Card, Empty, Tag, Button, Modal, Toast } from 'antd-mobile';
...
const OrderList = () => {
  ...
  const cancelMutation = useMutation({
    ...
    onError: (error) => {
      Toast.show({ content: error.message || '取消失败', icon: 'fail' });
    },
  });
          ...
          >
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: '#666', fontSize: '14px', marginTop: '8px' }}>
                ¥{order.payAmount} | {new Date(order.createdAt).toLocaleString()}
              </div>
            </div>
